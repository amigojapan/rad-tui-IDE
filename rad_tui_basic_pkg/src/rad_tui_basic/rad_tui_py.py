...
def main(stdscr):
    ...
    def launch_editor_json_for_code(target_ctrl_obj, form_ref, wins_list, glb_dict):
        try:
            ...
            editor_json_path = PACKAGE_DIR / "editor.json"
            with open(editor_json_path, 'r', encoding='utf-8') as f: ed_data = json.load(f)
    ...
    def launch_filedialogue_for_ide(mode, form_ref, wins_list, glb_dict):
        try:
            ...
            json_path = PACKAGE_DIR / "filedialogue.json"
...
def cli_entry():
    """Entry point function called by the rad-tui-ide executable command"""
    try:
        curses.wrapper(main)
    finally:
